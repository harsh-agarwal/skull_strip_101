...
class UnetModel(nn.Module):
    def __init__(
        self, in_channels, out_channels, model_depth=2, final_activation="sigmoid"
    ):
    ...
    def train(self, data_paths_loader, dataset_loader, batch_data_loader):
        ...
        for epoch in range(self.no_epochs):
            ...
            for step in range(training_steps):
                ...
                x_batch, y_batch = batch_data_loader(
                    pets, masks, iter_step=step, batch_size=self.batch_size
                )
            ...
            end_time = time.time()
            print(
                "Epoch {}, training loss {:.4f}, time {:.2f}".format(
                    epoch, train_losses / training_steps, end_time - start_time
                )
            )
